  ...
  d3.timeline = function() {
    ...
    function timeline (gParent) {
        ...
        g.each(function (d, i) {
          d.forEach(function (datum, index) {
            datum.times.forEach(function (time, j) {
              time.starting_time = parseInt(time.starting_time);
              time.ending_time = parseInt(time.ending_time);
              if(index === 0 && j === 0){
            ...
            datum.times.forEach(function (time, i) {
              ...
              if(ending === 0)
                if (time.ending_time > maxTime)
                  maxTime = time.starting_time+2000000;
            });
          });
        });
        ...
        d.forEach( function(datum, index){
          ...
          g.selectAll("svg").data(data).enter()
            ...
            .attr("width", function (d, i) {
              return (parseInt(d.ending_time) - parseInt(d.starting_time)) * scaleFactor;
            })
      ...
      function getXPos(d, i) {
        return margin.left + (parseInt(d.starting_time) - beginning) * scaleFactor;
      }

      function getXTextPos(d, i) {
        return margin.left + (parseInt(d.starting_time) - beginning) * scaleFactor + 5;
      }
